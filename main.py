...
# 'mysql+mysqlconnector://{user}:{password}@{server}/{database}'.format(user='your_user', password='password', server='localhost', database='dname')
db = SQLAlchemy(app)


class Contacts(db.Model):
    # sno,Name,email,phone-no,message
...
def uploader():
    # os module is used to save the uploaded file
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if (username == params['admin_user'] and userpass == params['admin_password']):
            # set the session variable for checking if already loogged in has to login again
...
# we need to pass variable into function like here we have post_slug
...
def edit(sno):
    ...
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            # serialno 0 is used for adding a new post #other serial no for upadating a existing
            if sno == '0':
